...
async function deliverToManager(contactId, person, text, env, leadId) {
  if (chatsEnabled(env) && contactId) {
    try {
      ...
      if (leadId) await env.S.put(`chatlead:${leadId}`, '1', { expirationTtl: 60 * 60 * 24 * 90 });
      return;
    } catch (e) {
...
async function onAmoNote(note, env, cfg) {
  ...
  // Текст примечания клиенту не уходит НИКОГДА — ни сам по себе, ни как
  // пояснение к команде. В примечаниях менеджеры пишут и внутренние заметки,
  // и один случайно отправленный клиенту комментарий дороже любого удобства.
  // Разговор идёт только через переписку карточки.
  if (!ask) {
    return writeLog(env, {
  ...
  // От команды остаётся только событие: у клиента открывается поле ввода,
  // текст пояснения не показывается.
  const at = Date.now();
  await env.S.put(msgKey(leadId, at), '', {
    expirationTtl: SESSION_TTL,
    metadata: { at, ask, note_id: noteId },
  });

  await writeLog(env, {
    verdict: `менеджер просит у клиента ${ask === 'inn' ? 'ИНН' : 'телефон'} (сделка ${leadId})`,
    lead_id: leadId, note_id: noteId, user_id: note.created_by,
  });
}
