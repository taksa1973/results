...
async function guessLevelWithModel(title, settings) {
  ...
  // Правила и примеры откалиброваны на реальной доске (26 из 30 заголовков):
  // без правила по словам модель тянула сложные задачи в «обычные»,
  // без примеров — простые. Короткое обоснование перед цифрой добавляет
  // секунды, но именно оно вытягивает сложные, которых нет в примерах.
  const prompt =
    `Оцени сложность задачи для личного ассистента.

` +
    `1 — простая: одно конкретное действие, понятно что делать, результат за час-два. ` +
    `Купить, заказать, скачать, оформить, пополнить одну конкретную вещь или услугу.
` +
    `2 — обычная: сравнить несколько вариантов или пройти несколько шагов, результат за день-два. ` +
    `Подобрать билет или технику, найти квартиру или специалиста по отзывам, уточнить условия.
` +
    `3 — сложная: исследование, план или система на несколько дней; много неизвестных, ` +
    `несколько стран, законов или источников.

` +
    `Правило: если задача просит понять, изучить, разобраться, спланировать, выяснить «как» или «где», ` +
    `настроить слежение или мониторинг — это 3, даже если сформулирована коротко. ` +
    `Если задача — купить, заказать, скачать или оформить одну конкретную вещь — это 1, ` +
    `даже если вещь необычная.
...
` +
    `Первая строка — одно короткое предложение: что придётся сделать. Вторая строка — только цифра уровня.`;
  ...
  try {
    ...
    if (!res.ok) return null;
    const data = await res.json().catch(() => ({}));
    // уровень — последняя цифра ответа: до неё идёт строка обоснования
    const m = String(data.response || '').match(/[123](?![\s\S]*[123])/);
